 ...
 * 网络诊断工具类 - 优化版本
 */
class NetworkDiagnosticsOptimized(
    ...
    suspend fun checkNetworkStatus(): NetworkStatus =
        networkStatusFlow.value ?: run {
            val status = checkNetworkStatusInternal()
            networkStatusFlow.value = status
            status
        }

    private suspend fun checkNetworkStatusInternal(): NetworkStatus =
     ...
     * 获取网络接口信息 - 优化版本
     */
    suspend fun getNetworkInterfaces(): List<NetworkInterfaceInfo> =
        withContext(Dispatchers.IO) {
            try {
