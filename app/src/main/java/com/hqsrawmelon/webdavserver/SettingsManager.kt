...
class SettingsManager(
    ...
    fun importSettings(jsonString: String): Boolean {
        try {
            ...
            settings.optString("username")?.let {
                if (it.isNotEmpty()) {
                    _username.value = it
                    editor.putString("username", it)
                }
            }
            settings.optString("password")?.let {
                if (it.isNotEmpty()) {
                    _password.value = it
                    editor.putString("password", it)
                }
            }
            if (settings.has("allow_anonymous")) {
                val value = settings.getBoolean("allow_anonymous")
                _allowAnonymous.value = value
                editor.putBoolean("allow_anonymous", value)
            }
            if (settings.has("server_port")) {
                val value = settings.getInt("server_port")
                _serverPort.value = value
                editor.putInt("server_port", value)
            }
            if (settings.has("enable_https")) {
                val value = settings.getBoolean("enable_https")
                _enableHttps.value = value
                editor.putBoolean("enable_https", value)
            }
            if (settings.has("connection_timeout")) {
                val value = settings.getInt("connection_timeout")
                _connectionTimeout.value = value
                editor.putInt("connection_timeout", value)
            }
            if (settings.has("max_connections")) {
                val value = settings.getInt("max_connections")
                _maxConnections.value = value
                editor.putInt("max_connections", value)
            }
            if (settings.has("buffer_size")) {
                val value = settings.getInt("buffer_size")
                _bufferSize.value = value
                editor.putInt("buffer_size", value)
            }
            if (settings.has("enable_cors")) {
                val value = settings.getBoolean("enable_cors")
                _enableCors.value = value
                editor.putBoolean("enable_cors", value)
            }
            if (settings.has("enable_compression")) {
                val value = settings.getBoolean("enable_compression")
                _enableCompression.value =
                    value
                editor.putBoolean("enable_compression", value)
            }
            if (settings.has("enable_ip_whitelist")) {
                val value = settings.getBoolean("enable_ip_whitelist")
                _enableIpWhitelist.value =
                    value
                editor.putBoolean("enable_ip_whitelist", value)
            }
            settings.optString("ip_whitelist")?.let {
                if (it.isNotEmpty()) {
                    _ipWhitelist.value = it
                    editor.putString("ip_whitelist", it)
                }
            }
            if (settings.has("max_failed_attempts")) {
                val value = settings.getInt("max_failed_attempts")
                _maxFailedAttempts.value = value
                editor.putInt("max_failed_attempts", value)
            }
            if (settings.has("block_duration")) {
                val value = settings.getInt("block_duration")
                _blockDuration.value = value
                editor.putInt("block_duration", value)
            }
            if (settings.has("enable_logging")) {
                val value = settings.getBoolean("enable_logging")
                _enableLogging.value = value
                editor.putBoolean("enable_logging", value)
            }
            settings.optString("log_level")?.let {
                if (it.isNotEmpty()) {
                    _logLevel.value = it
                    editor.putString("log_level", it)
                }
            }
            if (settings.has("max_log_size")) {
                val value = settings.getInt("max_log_size")
                _maxLogSize.value = value
                editor.putInt("max_log_size", value)
            }
